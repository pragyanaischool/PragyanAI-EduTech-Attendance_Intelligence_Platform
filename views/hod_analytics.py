...
def render_hod_analytics():
    ...
    semester-wise performance aggregations, and granular student multi-subject attendance tracking.
    """
    ...
    st.markdown(
        ...
        "Analyzing live database records for faculty workload pacing, semester performance, and granular multi-subject student attendance."
    )
    ...
    # --- TAB 3: STUDENT-WISE INTELLIGENCE & MULTI-SUBJECT DRILLDOWN ---
    with tab_student_audit:
        st.markdown("### 🎓 Student-Wise Attendance Turnout, Risk & Multi-Subject Drilldown")
        st.markdown("Filter students by Department, Semester, and Name to inspect live multi-subject attendance performance and exam eligibility.")
        ...
        if not stud_df.empty:
            ...
            if selected_student:
                ...
                st.markdown("#### 📚 Multi-Subject Attendance Breakdown & Status Audit")
                
                # Extract subject-wise attendance dictionary from database record
                subjects_dict = student_record.get("subjects", {})
                if subjects_dict:
                    subject_breakdown = []
                    for subj_name, metrics in subjects_dict.items():
                        subject_breakdown.append({
                            "Subject / Course": subj_name,
                            "Classes Held": metrics["held"],
                            "Classes Attended": metrics["attended"],
                            "Turnout %": f"{metrics['pct']}%",
                            "Academic Status": metrics["status"]
                        })
                    ...
                    fig_sub_att = px.bar(
                        ...
                        y="Classes Held",
                        text="Turnout %",
                        title=f"Multi-Subject Attendance Breakdown for {student_record['name']}",
                        color="Subject / Course",
                        color_discrete_sequence=px.colors.qualitative.Prism
                    )
                    st.plotly_chart(fig_sub_att, use_container_width=True)
                else:
                    st.info("No detailed subject attendance breakdown logged for this student.")
